...
fini=open("ips_find.txt","w")
fini.write("LISTA IPS:\n")
fini.close()
...
class Ping(object):
	...
	if lost_count==0:
		...
		try:
                        iplocalize.append(self.destination)
...
def verbose_ping(hostname, timeout=3000, count=3, packet_size=55):
...
if __name__ == '__main__':
	...
	for i in range(170,255):
		...
		for j in range(1,255):
			ips="169.254."+str(i)+"."+str(j)
			verbose_ping(ips,50,1)
			
		f = open ("ips_find.txt", "a")
		wri="recorrido bucle hasta: "+ips
                f.write(wri)
                f.close()

        print "*********************************************"
        print "*********************************************"
	print "*********** Ip encontradas ******************"
	print "*********************************************"
	print iplocalize
